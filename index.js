...
let hackerResult = hacker1.localeCompare(hacker2);

if (hackerResult){
    console.log("The driver's name goes first.");
}else if(hackerResult === -1){
...
let wordCounter = longText.split(/[\s,.;]+/);

     console.log(wordCounter.length)



// count "et" word in the longtext
let etCounter = 0;
for (let i=0; i<longText.length; i++)
{
    if(longText[i] === "e"){
...
let newPhrase = phraseToCheck.replace(/[\s \' , ! ?]+/g,"");
let newPhraseLower = newPhrase.toLowerCase()

let reversePhrase = '';

for(item = newPhraseLower.length - 1; item >= 0 ; item--){

    reversePhrase += newPhraseLower[item];
}

if(newPhraseLower === reversePhrase){
